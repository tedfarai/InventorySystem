      ...
      for (const item of items) {
        try {
          ...
          if (response.ok || response.status === 409) {
            ...
            tx.objectStore('sync_queue').delete(item.sync_id);
          }
        } catch {
          // Keep failed entries queued for the next sync attempt.
        }
      }
    }
  } catch {
    // The client-side database layer remains the source of truth.
  }
